...
uint8_t CC1100::get_payload_ori(uint8_t rxbuffer[], uint8_t &pktlen, uint8_t &my_addr,
...
//------------------[check Payload for ACK or Data] MOd version-----------------------------



//  not printf. output to *buffer for sqlite3
void CC1100::get_payload(char *oneMSG, uint8_t rxbuffer[], uint8_t &pktlen, uint8_t &my_addr,
                            uint8_t &sender, int8_t &rssi_dbm, uint8_t &lqi)
{
    uint8_t crc;
    char buffer[100];
    ...
    else
    {
        ...
        }else                                               //real data, and sent acknowladge
        {
          ...
          lqi = lqi_convert(rxbuffer[pktlen + 2]);       //get rf quialtiy indicator
          crc = check_crc(lqi);                          //get packet CRC
          sprintf(buffer ,"RX_FIFO:");
          strcat(oneMSG,buffer);

          for(uint8_t i = 0 ; i < pktlen + 1; i++)   //showes rx_buffer for debug
          {
          sprintf(buffer ,"0x%02X ", rxbuffer[i]);
          strcat(oneMSG,buffer);
          }

          sprintf(buffer ,"| 0x%02X 0x%02X |", rxbuffer[pktlen+1], rxbuffer[pktlen+2]);
          strcat(oneMSG,buffer);
          sprintf(buffer ,"RSSI:%d ", rssi_dbm);
          strcat(oneMSG,buffer);
          sprintf(buffer ,"LQI:");
          strcat(oneMSG,buffer);
          sprintf(buffer ,"0x%02X ", lqi);
          strcat(oneMSG,buffer);
          sprintf(buffer ,"CRC:");
          strcat(oneMSG,buffer);
          sprintf(buffer ,"0x%02X ", crc);
          strcat(oneMSG,buffer);
          puts("\r\n");
